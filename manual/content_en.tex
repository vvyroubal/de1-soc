...
the board (\code{-{}-sysroot}). The image ships \code{gcc} and \code{make}
preinstalled, so you can also compile small programs natively on the board
(add \code{apt install build-essential} for the full toolset).
\end{note}
...
\begin{note}[Runtime reconfiguration from Linux (no reboot)]
You can also reconfigure the fabric at runtime from the HPS. Mainline Linux has no
built-in userspace overlay interface, so this image autoloads the \code{dtbocfg}
module (providing \code{/sys/kernel/config/device-tree/overlays}) and ships
\code{/usr/local/sbin/fpga-load.sh}. It stages an uncompressed \code{.rbf} to
\code{/lib/firmware} and applies a device-tree overlay to
\code{/soc/base\_fpga\_region}, so the FPGA Manager programs the fabric:
\begin{lstlisting}
sudo /usr/local/sbin/fpga-load.sh my_design.rbf
cat /sys/class/fpga_manager/fpga0/state    # -> operating
\end{lstlisting}
The \code{.rbf} must be uncompressed either way. See \code{fpga/fpga-load/}.

Runtime reconfiguration works for \emph{self-contained} designs. A design that
exposes memory-mapped slaves to the HPS through the FPGA bridges must instead be
loaded \emph{at boot} (place its \code{.rbf} on the FAT partition as
\code{soc\_system.rbf}): a runtime reconfiguration swaps the fabric out from
under the already-initialised bridges and the HPS then hangs on the first access
to a fabric slave.
\end{note}
