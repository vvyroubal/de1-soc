...
pločice (\code{-{}-sysroot}). Slika dolazi s predinstaliranim \code{gcc} i
\code{make}, pa male programe možete prevoditi izravno na pločici (za puni
komplet alata dodajte \code{apt install build-essential}).
\end{note}
...
\begin{note}[Rekonfiguracija iz Linuxa tijekom rada (bez ponovnog podizanja)]
Programabilnu logiku možete rekonfigurirati i tijekom rada iz HPS-a. Mainline Linux
nema ugrađeno korisničko sučelje za overlaye, pa ova slika automatski učitava modul
\code{dtbocfg} (koji daje \code{/sys/kernel/config/device-tree/overlays}) i sadrži
\code{/usr/local/sbin/fpga-load.sh}. On sprema nekomprimirani \code{.rbf} u
\code{/lib/firmware} i primjenjuje device-tree overlay na
\code{/soc/base\_fpga\_region}, pa FPGA Manager programira logiku:
\begin{lstlisting}
sudo /usr/local/sbin/fpga-load.sh my_design.rbf
cat /sys/class/fpga_manager/fpga0/state    # -> operating
\end{lstlisting}
\code{.rbf} mora biti nekomprimiran u svakom slučaju. Vidi \code{fpga/fpga-load/}.

Rekonfiguracija tijekom rada radi za \emph{samostalne} dizajne. Dizajn koji
izlaže memorijski mapirane podređene jedinice HPS-u preko FPGA mostova mora se
umjesto toga učitati \emph{pri pokretanju} (stavite njegov \code{.rbf} na FAT
particiju kao \code{soc\_system.rbf}): rekonfiguracija tijekom rada zamijeni
fabric ispod već inicijaliziranih mostova, pa se HPS zablokira pri prvom
pristupu podređenoj jedinici u fabricu.
\end{note}
